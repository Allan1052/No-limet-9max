"""
Card IMPACTANTE: imagem split-screen (ônibus/mesa final) + logo + frase de impacto.
Layout no estilo dos cards anteriores: logo no topo, imagem, frase impactante embaixo.
"""
...
def create_card():
    ...
    # === LOGO (monograma em círculo dourado) ===
    ...
    # === FRASE IMPACTANTE ===
    text_area_y = logo_space + art.height - 50
    ...
    main_font = ImageFont.truetype(FONT_BOLD, 40)
    main_lines = wrap_text(MAIN_PHRASE, main_font, WIDTH - 100, draw)
    line_h = 52
    ...
    # Sub-frase (a provocação)
    text_area_y += 12
    sub_font = ImageFont.truetype(FONT_ITALIC, 24)
    draw.text(
        (WIDTH // 2, text_area_y),
        SUB_PHRASE,
        fill=(180, 180, 160),
    ...
    # Salvar
    out_path = "/home/ubuntu/no-limet-9max/card-impactante-final.png"
    final.save(out_path, "PNG", quality=95)
    print(f"Card impactante salvo: {out_path} ({WIDTH}x{HEIGHT})")
