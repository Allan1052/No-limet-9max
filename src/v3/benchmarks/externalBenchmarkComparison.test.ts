...
describe("Motor V3 — V2 × V3 × solver external comparison", () => {
  it("classifies all certified fixtures without inventing V2 exact comparability", () => {
    expect(EXTERNAL_BENCHMARK_COMPARISON).toHaveLength(19);
  ...
  it("preserves the exact solver-side benchmark identifiers across all seven families", () => {
    expect(EXTERNAL_BENCHMARK_COMPARISON.map((row) => row.id).sort()).toEqual([
      ...
      "BW5",
      "FTBB1_SYMMETRIC_35",
      "FTBB2_BB_COVERS_40_VS_35",
      "FTBB3_SB_COVERS_100_VS_35",
  ...
  it("keeps final-table blind battles structural-only in the comparison harness", () => {
    const finalTableRows = EXTERNAL_BENCHMARK_COMPARISON.filter(
      (row) => row.family === "FINAL_TABLE_BLIND_BATTLE",
    );

    expect(finalTableRows).toHaveLength(3);
    expect(finalTableRows.map((row) => row.id)).toEqual([
      "FTBB1_SYMMETRIC_35",
      "FTBB2_BB_COVERS_40_VS_35",
      "FTBB3_SB_COVERS_100_VS_35",
    ]);
    for (const row of finalTableRows) {
      expect(row.v3.status).toBe("EXACT_STRUCTURAL_FIXTURE");
      expect(row.v3.handLevelCertified).toBe(false);
    }
  });
});
