...
//   npx vitest run src/sim/_pressaoRun.test.ts
...
describe("pressão do campo por faixa de buy-in", () => {
  it("joga torneios completos e mede o que o campo faz com o jogador", () => {
    const linhas: string[] = [];
    linhas.push(
      "buyIn torneios  maos  dec. 3betVsOpen cbetVsEle apostas/100(F/T/R)  ckR/100 allin/100 apertados% degraca% comMao% ITM% posMed",
    );
